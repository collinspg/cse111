## I exceeded requirements by adding a test function for my kpa_to_psi_conversion 
# import form the water_flow program to create test functions, import approx from pytest and also 
#import pytest.
from water_flow_2 import water_column_height, pressure_gain_from_water_height, pressure_loss_from_pipe, reynolds_number, pressure_loss_from_pipe_reduction, pressure_loss_from_fittings, kpa_to_psi
...
##Exceeding requirements test-codes       
# A test function for kpa_to_psi_conversion        
def test_kpa_to_psi_conversion():
    test_cases = [
        # Test case: kPa input, expected psi output
        (0, 0),
        (100, 14.5037737732),
        (200, 29.0075475464),
        (500, 72.518868866),
    ...
    for i, (kpa, expected_psi) in enumerate(test_cases):
        result_psi = kpa_to_psi(kpa)
        assert result_psi == approx(expected_psi, abs=0.01)   
